...
호수#비밀번호#
즉 호수를 누르고 '#'을 누른 후 비밀번호를 누르고 다시 '#'을 누르면 공동현관문이 열리게 됩니다. 
예를 들어 1201호에서 1234라는 비밀번호를 사용하고 있다면 다음과 같이 공동 현관문을 열 수 있습니다.
1201#1234#
...
잘못된 호수를 입력한 경우
잘못된 비밀번호를 입력한 경우
본 문제에서는 XX아파트에서 공동현관 출입을 위해 입력한 내용이 한 줄로 된 문자열 형태로 주어질 때, 올바르게 비밀번호를 입력하여 공동현관문을 연 횟수는 총 몇 번인지 구하면 됩니다.
 문자열로 주어진 내용은 앞에서부터 순서대로 공동현관문에 입력되었다고 가정합니다.
...
public class Ex01 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.solution(passwords, s));
    }
    class Solution {
        ...
        public int solution(int[][] passwords, String s) {
            ...
            for(int i = 0; i < passwords.length; i++){
                // passwords[i][0] // 호수
                // passwords[i][1] // 비밀번호
                pwdMap.put(String.valueOf(passwords[i][0]), String.valueOf(passwords[i][1]));      
            }
            ...
            while(st.hasMoreTokens()){
                ...
                //현관문 호수는 존재 비밀번호 비교시작
                if(pwdMap.containsKey(firstInput)){
                    String inputPwd = "";
                    if(st.hasMoreTokens()){
                        inputPwd = st.nextToken();
                    }
                    //비밀번호가 일치한다면
                    if(inputPwd.equals(pwdMap.get(firstInput))){
                        answer += 1;
                    }
                }
                //현관문 호수에 값이 존재하지 않을때
